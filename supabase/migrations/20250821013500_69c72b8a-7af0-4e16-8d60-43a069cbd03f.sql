-- Create corrected unified views for search functionality
CREATE OR REPLACE VIEW public.unified_shipments_view AS
SELECT
  us.id,
  us.org_id,
  CASE 
    WHEN LOWER(COALESCE(us.mode, us.shipment_type, 'ocean')) LIKE '%air%' THEN 'air'::text 
    ELSE 'ocean'::text 
  END as mode,
  ...
  us.destination_city,
  COALESCE(us.unified_carrier, us.carrier_name) as unified_carrier,
  COALESCE(us.unified_date, us.shipment_date, us.arrival_date)::date as unified_date,
  COALESCE(us.commodity_description, us.description) as description,
  COALESCE(us.bol_number, us.bill_of_lading_number) as bol_number,
  us.vessel_name,
  us.gross_weight_kg,
  us.value_usd,
  us.container_count,
  ...
  to_tsvector('english', 
    ...
    COALESCE(us.commodity_description, us.description, '') || ' ' ||
    COALESCE(us.hs_code, '') || ' ' ||
    COALESCE(us.unified_carrier, us.carrier_name, '')
  ) as ts_all
FROM public.unified_shipments us;

-- Create search companies view with aggregated data
CREATE OR REPLACE VIEW public.search_companies_view AS
WITH ship_stats AS (
  SELECT
    usv.unified_company_name as company_name,
    COUNT(*)::int as shipments_count,
    MAX(usv.unified_date) as last_shipment_date,
    ARRAY_AGG(DISTINCT usv.mode) FILTER (WHERE usv.mode IS NOT NULL) as modes,
    ARRAY_AGG(DISTINCT usv.destination_country) FILTER (WHERE usv.destination_country IS NOT NULL) as dest_countries,
    ARRAY_AGG(DISTINCT usv.hs_code) FILTER (WHERE usv.hs_code IS NOT NULL) as top_commodities
  FROM public.unified_shipments_view usv
  WHERE usv.unified_company_name IS NOT NULL
  GROUP BY usv.unified_company_name
),
contact_stats AS (
  SELECT 
    cc.company_name, 
