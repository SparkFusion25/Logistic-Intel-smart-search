-- Fix the migrate_legacy_shipments() function to use only existing columns
...
BEGIN
  ...
  SELECT 
    ...
    NULLIF(TRIM(COALESCE(commodity_description, goods_description, '')), '') as commodity_description,
    NULLIF(TRIM(COALESCE(bol_number, '')), '') as bol_number,
  ...
  SELECT 
    ...
    NULLIF(TRIM(COALESCE(commodity_description, goods_shipped, '')), '') as commodity_description,
