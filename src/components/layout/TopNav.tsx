...
export default function TopNav(){
  ...
  const tab = (href:string, label:string)=> (
    <Link to={href} className={`px-3 py-1.5 rounded-xl border text-sm hover:bg-white/10 ${r.pathname===href?'bg-white/10 border-white/20':'bg-white/5 border-white/10'}`}>{label}</Link>
  );
  return (
