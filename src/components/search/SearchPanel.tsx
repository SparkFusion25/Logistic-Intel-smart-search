...
function ModeToggle({ mode, setMode }:{ mode:Mode; setMode:(m:Mode)=>void }){
  const Btn=(m:Mode,label:string)=>(<button onClick={()=>setMode(m)} className={`px-3 py-1.5 rounded-full border ${mode===m?'bg-blue-600 border-blue-400':'bg-white/5 border-white/10 hover:bg-white/10'}`}>{label}</button>);
  return (<div className="flex items-center gap-2">{Btn('all','All')}{Btn('air','Air ✈')}{Btn('ocean','Ocean 🚢')}</div>);
}
...
export default function SearchPanel(){
  ...
  const onAddToCrm=async(row:UnifiedRow)=>{ try{ await fetch('/api/crm/contacts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ company_name:row.unified_company_name, mode:row.mode, hs_code:row.hs_code, origin_country:row.origin_country, destination_country:row.destination_country, destination_city:row.destination_city, carrier:row.unified_carrier, bol_number:row.bol_number, vessel_name:row.vessel_name, last_seen:row.unified_date, source:'search_unified' })}); }catch{} };
  const onApplyFilters=()=>run(true); const onClearFilters=()=>run(true);
  return (
    <div className="flex flex-col gap-4">
      ...
      <AIAssistBar q={q} filters={filters} lastResults={items} onPickSuggestion={(s)=>{setQ(s); run(true);}} onApplyStructured={(f)=>{ setFilters((x)=>({ ...(x as Filters), ...(f||{}) })); run(true); }}/>
      ...
      <div className="flex items-center justify-between">
        ...
        {hasMore&&(<button onClick={loadMore} disabled={loading} className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-sm">Load more</button>)}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">{items.map((r)=>(<ResultRow key={r.id} r={r} q={q} onAddToCrm={onAddToCrm}/>))}</div>
    </div>
  );
}