  ...
  const Btn = (m: Mode, label: string) => (
    <button
      ...
      className={`px-3 py-1.5 rounded-full border ${
        mode === m ? 'bg-blue-600 border-blue-400' : 'bg-white/5 border-white/10 hover:bg-white/10'
      }`}
    >
      {label}
    </button>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-3 md:p-4 flex flex-col gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs opacity-85">
        <div>
          <span className="opacity-60">HS:</span> {r.hs_code || '—'}
        </div>
        <div>
          <span className="opacity-60">Origin:</span> {r.origin_country || '—'}
        </div>
        <div>
          <span className="opacity-60">Dest:</span> {r.destination_city || r.destination_country || '—'}
        </div>
        <div>
          <span className="opacity-60">Date:</span> {r.unified_date || '—'}
        </div>
      </div>
      <div className="text-xs line-clamp-2 opacity-80">{r.description || 'No description'}</div>
      <div className="flex items-center gap-2 pt-1">
        <button
          className="rounded-xl bg-white/5 border border-white/10 px-2 py-1 text-xs hover:bg-white/10"
          onClick={() => onAddToCrm(r)}
        >
          Add to CRM
        </button>
        <button className="rounded-xl bg-white/5 border border-white/10 px-2 py-1 text-xs hover:bg-white/10">
          Export
        </button>
      </div>
    </div>
...
export default function SearchPanel() {
  const { q, setQ, mode, setMode, filters, setFilters, items, total, loading, error, hasMore, run, loadMore } =
    useUnifiedSearch({ initialMode: 'all', initialLimit: 25 });

  // Add-to-CRM (stub hits API; swap for Supabase insert when ready)
  const onAddToCrm = async (row: UnifiedRow) => {
    try {
      await fetch('/pages/api/crm/contacts'); // noop for environments without the file path
    } catch {}
    try {
      ...
      await r.json();
      // Optional: toast/notify success
    } catch (e) {
      // Optional: toast/notify failure
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
        ...
        <button
          onClick={() => run(true)}
          className="px-3 py-2 rounded-2xl bg-blue-600 hover:bg-blue-500 disabled:opacity-60"
          disabled={loading}
        >
          Search
        </button>
      ...
      {/* AI Assist (Smart suggestions + structured filter apply) */}
      <AIAssistBar
        q={q}
        filters={filters}
        lastResults={items}
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm opacity-75">
          {loading ? 'Loading…' : error ? `Error: ${error}` : `${total} results`}
        </div>
        {hasMore && (
          <button
            onClick={loadMore}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {items.map((r) => (
          <ResultRow key={r.id} r={r} q={q} onAddToCrm={onAddToCrm} />
        ))}
      </div>
    </div>
  );
